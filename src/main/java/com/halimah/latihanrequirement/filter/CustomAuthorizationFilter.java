package com.halimah.latihanrequirement.filter;
...
public class CustomAuthorizationFilter extends OncePerRequestFilter {
    ...
    private static final String HTTP_OPTION_METHOD = "OPTIONS";
    private  final TokenProvider tokenProvider;
    ...
    private Map<String, String> getRequestValue(HttpServletRequest request) {
        return of(EMAIL_KEY,tokenProvider.getSubject(getToken(request),request),TOKEN_KEY,getToken(request));
    }
    private String getToken(HttpServletRequest request) {
        return ofNullable(request.getHeader(AUTHORIZATION))
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
        return request.getHeader(AUTHORIZATION)== null ||
        !request.getHeader(AUTHORIZATION).startsWith(TOKEN_PREFIX) ||
        request.getMethod().equalsIgnoreCase(HTTP_OPTION_METHOD)||
        asList(PUBLIC_ROUTES).contains(request.getRequestURI());
    }
}
